...
def get_images_t(image_dir, preprocess='0', phase='train'):
    ...
    for image_path in image_paths:
        ...
        name = os.path.split(image_path)[1].split('.')[0]
        for lesion, lesion_abbv in zip(lesions, lesion_abbvs):
            if preprocess == '3' and phase == 'train':
                # candidate_path = os.path.join(mask_path, lesion, name + '_' + lesion_abbv + '.png')
                candidate_path = os.path.join(mask_path, lesion, name + lesion_abbv + '.png')
            else:
                candidate_path = os.path.join(mask_path, lesion, name + '_' + lesion_abbv + '.tif')
            if os.path.exists(candidate_path):
                paths.append(candidate_path)
...
def get_images_ddr(image_dir, preprocess='0', phase='train'):
    ...
    for image_path in image_paths:
        ...
        for lesion, lesion_abbv in zip(lesions, lesion_abbvs):
            # candidate_path = os.path.join(mask_path, lesion, name + '_' + lesion_abbv + '.tif')
            candidate_path = os.path.join(mask_path, lesion_abbv, name + '.tif')
            if os.path.exists(candidate_path):
                paths.append(candidate_path)
            else:
                paths.append(None)
