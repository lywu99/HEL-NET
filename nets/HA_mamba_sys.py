...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.d_model = d_model               #输入序列的特征大小
        self.d_state = d_state               #状态转移矩阵的特征大小
        # self.d_state = math.ceil(self.d_model / 6) if d_state == "auto" else d_model # 20240109
        self.d_conv = d_conv                 #一维卷积的步长，默认4
        self.expand = expand                #输入特征的扩展倍数
        self.d_inner = int(self.expand * self.d_model)
        self.dt_rank = math.ceil(self.d_model / 16) if dt_rank == "auto" else dt_rank                #离散化的步长总数
...
class HA_VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.conv1 = nn.Conv2d(hidden_dim, hidden_dim*2,kernel_size=1)
        self.ln_1 = norm_layer(hidden_dim)
        self.self_attention = SS2D(d_model=hidden_dim, dropout=attn_drop_rate, d_state=d_state, **kwargs)
        self.drop_path = DropPath(drop_path)
        ...
        self.HeterConv = HeterogeneousConv(in_channels=hidden_dim, out_channels=hidden_dim, kernel_sizes=[7, 11, 21])
        # self.HeterConv = HeterogeneousConv(in_channels=hidden_dim, out_channels=hidden_dim, kernel_sizes=[5, 9, 17])

        self.final_conv1 = nn.Conv2d(hidden_dim*2, hidden_dim, kernel_size=1)
    def forward(self, input: torch.Tensor):

        B, H, W, C = input.shape
        inputs = self.conv1(input.permute(0, 3, 1, 2))
        x1, x2 = inputs.chunk(2, dim=1)

        x1 = x1.permute(0, 1, 2, 3).contiguous()
        x2 = x2.permute(0, 2, 3, 1).contiguous()

        y1 = self.HeterConv(self.bn_1(x1))
        y1 = y1.permute(0, 2, 3, 1)
        y2 = self.drop_path(self.self_attention(self.ln_1(x2)))
        y = torch.cat((y1, y2), dim=-1)
        y = self.final_conv1(y.permute(0, 3, 1, 2))
        x = input + y.permute(0, 2, 3, 1)
        # x = input + self.drop_path(self.self_attention(self.ln_1(input)))

        return x

class HA_VSSBlock_v1(nn.Module):
    def __init__(
        self,
        hidden_dim: int = 0,
        drop_path: float = 0,
        ...
        **kwargs,
        ...
        # self.HeterConv = HeterogeneousConv(in_channels=hidden_dim, out_channels=hidden_dim, kernel_sizes=[7, 11, 21])
        # self.HeterConv = HeterogeneousConv(in_channels=hidden_dim, out_channels=hidden_dim, kernel_sizes=[5, 9, 17])
        self.HeterConv = HeterogeneousConv_v1(in_channels=hidden_dim, out_channels=hidden_dim,
                                           kernel_sizes=[7, 11, 21],kernel_sizes1=[5, 9, 17])
    ...
    def forward(self, input: torch.Tensor):
        ...
        # inputs = self.conv1(input.permute(0, 3, 1, 2))
        # x1, x2 = inputs.chunk(2, dim=1)
        # x1 = input.permute(0, 1, 2, 3).contiguous()
        # x2 = x2.permute(0, 2, 3, 1).contiguous()
        ...
        y2 = self.drop_path(self.self_attention(self.ln_1(input)))
        y = torch.cat((y1, y2), dim=-1)
        y = self.final_conv1(y.permute(0, 3, 1, 2))
        x = input + y.permute(0, 2, 3, 1)
        # x = input + self.drop_path(self.self_attention(self.ln_1(input)))
        ...
        #另一种方式
        # attention_scores = self.attention_conv(y1)
        # attended_features = input * attention_scores
        # output = input + attended_features
        return x


class HA_VSSBlock_v2(nn.Module):
    def __init__(
        self,
        hidden_dim: int = 0,
        drop_path: float = 0,
    ...
    def forward(self, input: torch.Tensor):
        ...
        y1 = self.HeterConv(self.bn_1(x1))
        attention_scores = self.attention_conv(y1)
        attended_features = x1 * attention_scores

        att_y1 = attended_features.permute(0, 2, 3, 1)

        y2 = self.drop_path(self.self_attention(self.ln_1(input)))

        #两种方案
        y = torch.cat((att_y1, y2), dim=-1)
        y = self.final_conv1(y.permute(0, 3, 1, 2))
        x = input + y.permute(0, 2, 3, 1)
        ...
        # #第三种方案
        # y2 = self.drop_path(self.self_attention(self.ln_1(att_y1)))
        # x = input + self.drop_path(self.self_attention(self.ln_1(att_y1)))
...
class HA_VSSLayer_up(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    Args:
        dim (int): Number of input channels.
        depth (int): Number of blocks.
    ...
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        for i_layer in range(self.num_layers):
            ...
            # layer=HA_VSSLayer(
                ...
                d_state=math.ceil(dims[0] / 6) if d_state is None else d_state, # 20240109  #状态转移矩阵的特征大小，默认16
                drop=drop_rate, 
